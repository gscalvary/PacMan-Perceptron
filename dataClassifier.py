...
def enhancedFeatureExtractorDigit(datum):
    ...
    "*** YOUR CODE HERE ***"
    # set-up a goal list for a depth first search
    goalList = []
    
    for x in range(DIGIT_DATUM_WIDTH):
        goalList.append((x, DIGIT_DATUM_HEIGHT))
        goalList.append((x, 0))

    for y in range(DIGIT_DATUM_HEIGHT):
        goalList.append((0, y))
        goalList.append((DIGIT_DATUM_WIDTH, y))
    
    goal = set(goalList)

    # per horizontal line boolean for multiple strokes
    for x in range(DIGIT_DATUM_WIDTH):
        transitionCount = 0
        priorPixel = 0
        for y in range(DIGIT_DATUM_HEIGHT):
            if features[(x, y)] != priorPixel:
                priorPixel = features[(x, y)]
                transitionCount += 1
                # trapped pixels inside loop (no path to edge image)
                if features[(x, y)] == 0:
                    if depthFirstSearch(features, (x, y), goal):
                        features[(x, y, "trapped")] = 0
                    else:
                        features[(x, y, "trapped")] = 1
        if transitionCount >= 4:
            features[(x, "multistroke")] = 1
        else:
            features[(x, "multistroke")] = 0

    # per vertical line boolean for multiple strokes
    for y in range(DIGIT_DATUM_HEIGHT):
        transitionCount = 0
        priorPixel = 0
        for x in range(DIGIT_DATUM_WIDTH):
            if features[(x, y)] != priorPixel:
                ...
                transitionCount += 1
        if transitionCount >= 4:
            features[("multistroke", y)] = 1
        else:
            features[("multistroke", y)] = 0

    return features


def depthFirstSearch(features, node, goal):
    ...
    while True:
        ...
        if exploreNode not in exploredSet:
            if exploreNode in goal:
                return True
            exploredSet.add(exploreNode)
            for child in successors(features, exploreNode):
                if (child not in exploredSet):
                    frontier.push(child)


def successors(features, node):
    children = []
    # check left of node
    if node[0] - 1 >= 0:
        if features[(node[0] - 1, node[1])] == 0:
            children.append((node[0] - 1, node[1]))
    # check right of node
    if node[0] + 1 <= DIGIT_DATUM_WIDTH:
        if features[(node[0] + 1, node[1])] == 0:
            children.append((node[0] + 1, node[1]))
    # check above node
    if node[1] + 1 <= DIGIT_DATUM_HEIGHT:
        if features[(node[0], node[1] + 1)] == 0:
...
def enhancedPacmanFeatures(state, action):
    ...
    # baseline:
    # 84% contest agent, 72% suicide agent
    nextState = state.generateSuccessor(0, action)

    # distance to nearest food feature
    closestDist = float("inf")
    pacPos = nextState.getPacmanPosition()
    foodGrid = nextState.getFood()
    for foodPos in foodGrid.asList():
        dist = util.manhattanDistance(foodPos, pacPos)
        if dist < closestDist:
            closestDist = dist
    if closestDist == float("inf"):
        closestDist = 0
    features["foodDist"] = closestDist

    # distance to nearest ghost feature
    closestDist = float("inf")
    pacPos = nextState.getPacmanPosition()
    for ghostPos in nextState.getGhostPositions():
        dist = util.manhattanDistance(ghostPos, pacPos)
        if dist < closestDist:
            closestDist = dist
    if closestDist == float("inf"):
        closestDist = 0
    features["ghostDist"] = closestDist
